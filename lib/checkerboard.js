...
async function measureCheckerboard(buffer, cfg) {
	...
	// otsuThreshold's class B is "value <= threshold", so foreground must be
	// selected inclusively here - a strict "<" would
	...
	const mmPerPixel = cfg.targetPitchMm / ((pitchXPx + pitchYPx) / 2);
...
module.exports = { measurePitch, measureCheckerboard };
