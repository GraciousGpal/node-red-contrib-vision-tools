...
function loadNative() {
	...
	if (!NATIVE_PLATFORMS.has(platformId)) {
		throw new Error(
...
function select(order) {
	...
	for (const name of order) {
		try {
			const engine = name === "native" ? loadNative() : loadCvjs();
...
function candidate() {
	...
	if (candidateCache === undefined) candidateCache = select(order());
...
function resolve() {
	...
	resolvePromise = (async () => {
		...
		if (first.name === "native") {
			try {
				...
				const demoted = mode() === "native" ? null : select(["opencv-js"]);
				resolvedCache =
					demoted && demoted.engine
						? demoted
	...
	// The IIFE above encodes every failure in its resolved value and never
	// rejects, so this cannot fire today. It is kept deliberately: if resolve()
	// is ever changed to propagate an error instead, a settled rejected promise
	// would otherwise be cached forever and no later call could retry. The twin
	// guard in lib/cvjs.js ready() is load-bearing right now, for that reason.
