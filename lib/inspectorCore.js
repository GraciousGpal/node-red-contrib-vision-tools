...
const { rectifyParallel } = require("./parallel.js");
...
/**
 * perspective-rectify's warp, split across the nested pool like the
 * golden warp is (`workers` sizes it the same way golden-compare's does;
 * 0 = auto). `frame` is the pixel handle (a SharedArrayBuffer on the
 * worker path, so nothing is copied either way) and the geometry rides
 * alongside; the output is allocated shared by the warp itself and its
 * handle goes back the same way.
 */
async function rectify({ frame, width, height, channels, homography, workers }) {
	const out = await rectifyParallel(
		{ data: new Uint8Array(frame), width, height, channels },
		homography,
		workers,
	);
	return {
...
module.exports = { prepare, inspect, calibrate, rectify, clear, MAX_GOLDENS };
