 ...
 */
const INLINE = process.env.GOLDEN_COMPARE_INLINE === "1" || !HAS_SAB;
...
// A rendered image is either an encoded Buffer or, under
// heatmapFormat "raw", a { data, width, height, ... } object whose data
// needs the same re-wrap.
function asImage(value) {
	if (value == null || Buffer.isBuffer(value)) return value;
	if (value.data != null) return { ...value, data: asBuffer(value.data) };
...
function rewrapResult(result) {
	...
	if (result.printBlemish) {
		result.printBlemish.heatmap = asImage(result.printBlemish.heatmap);
	}
	if (result.backgroundBlemish) {
		result.backgroundBlemish.heatmap = asImage(
			result.backgroundBlemish.heatmap,
		);
	}
	if (result.stages) {
		for (const key of Object.keys(result.stages)) {
			result.stages[key] = asImage(result.stages[key]);
		}
	}
	return result;
}
...
/**
 * Warp a raw frame through a homography. `frame` is a zero-offset
 * SharedArrayBuffer-backed view (see toShared); the reply's `data` is a
 * Buffer over shared memory, not a copy.
 */
async function rectify({ frame, width, height, channels, homography, workers }) {
	const reply = await call("rectify", {
		frame: frame.buffer,
		width,
		height,
		channels,
	...
	const { data, ...rest } = reply.result;
	return { result: { data: Buffer.from(data, 0, data.byteLength), ...rest } };
}
...
module.exports = { prepare, inspect, calibrate, rectify, shutdown, INLINE };
