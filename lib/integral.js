...
function fillIntegral(integral, src, width, height) {
...
function buildIntegral(src, width, height) {
	...
	const integral = allocU32((width + 1) * (height + 1));
	return fillIntegral(integral, src, width, height);
}
...
function buildIntegral64(src, width, height) {
	const integral = allocF64((width + 1) * (height + 1));
	return fillIntegral(integral, src, width, height);
}
