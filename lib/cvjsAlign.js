...
const { scope, toMat, fromMat, describeRaw } = cvjs._internals;
...
/** opencv.js throws heap pointers, not Errors. */
function cvMessage(cv, err) {
	if (typeof err === "number" && typeof cv.exceptionFromPtr === "function") {
		try {
			return cv.exceptionFromPtr(err).msg;
		} catch {
			return `opencv exception ${err}`;
		}
	}
	return err && err.message ? err.message : String(err);
}

function grayMat(cv, s, image) {
	...
	if (code === undefined) throw new Error(`cvjs: cannot take ${raw.colorSpace} to GRAY`);
...
async function imageAlign(
	...
	const s = scope();
	try {
		const refFull = grayMat(cv, s, reference);
		const targetGray = grayMat(cv, s, target);
		...
		if (!m) {
			return {
				...
				image: null,
				timing: { totalMs: performance.now() - t0, engine: "opencv-js", op: "imageAlign" },
			};
		}
...
module.exports = { imageAlign, MIN_MATCHES, MAX_MATCHES };
