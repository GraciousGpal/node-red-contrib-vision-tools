 ...
 *     the result.
 *
 * The result carries, hit or miss, one `caliperLines` entry per band -
 * where that caliper looked, the strongest step it saw, and the edge it
 * chose - and a `diagnostics` block with the strongest and median
 * per-caliper contrast in the same units as `contrastThreshold`. A miss
 * then explains itself: the editor can say "the strongest step here is
 * 1.3, the threshold is 2" instead of only "no-edge".
 *
...
/**
 * The steepest derivative in a smoothed profile, signed like `strength`
 * (positive = brighter along the scan), over the same samples findEdges
 * considers. This is the number an operator has to beat with
 * contrastThreshold, so it is reported whether or not anything qualified.
 * Zero for a profile too short to hold a candidate.
 */
function peakGradient(profile) {
	const n = profile.length;
	let best = 0;
	for (let i = 2; i < n - 2; i++) {
		const v = (profile[i + 1] - profile[i - 1]) / 2;
 ...
 *   residualPx, points, caliperLines, diagnostics }
 *
 *   caliperLines: one per band, in image coordinates - { band, p0, p1,
 *     complete, peakContrast, edge, used }, where p0/p1 are the ends of
 *     the caliper along the scan axis, `complete` says whether the band
 *     was wholly inside the image, `peakContrast` is the steepest step
 *     it saw (null when not complete), `edge` the point it chose or
 *     null, and `used` whether that point survived the outlier trim.
 *   diagnostics: { peakContrast, peakPolarity, medianPeakContrast,
 *     calipersWithEdge, calipersInImage } - the strongest step in any
 *     caliper and its direction, the median of the per-caliper peaks,
 *     how many calipers reported a qualifying edge, and how many were
...
function findLine(gray, width, height, region, cfg = {}) {
	...
	const points = [];
	// one entry per band whether or not it found anything, so a preview can
	// show where every caliper looked and the strongest step each one saw
	const caliperLines = new Array(bands);
	const peaks = [];
	let peakContrast = 0;
	let peakStrength = 0;
	let found = 0;
	for (let b = 0; b < bands; b++) {
		...
		const startX = frame.origin.x + frame.line.x * v;
		const startY = frame.origin.y + frame.line.y * v;
		const caliper = {
			band: b,
			p0: { x: startX, y: startY },
			p1: {
				x: startX + frame.scan.x * frame.depth,
				y: startY + frame.scan.y * frame.depth,
			},
			complete: false,
			peakContrast: null,
			edge: null,
		...
		caliperLines[b] = caliper;
		// null means the band was not wholly inside the image
		const profile = profiles[b];
		if (!profile) continue;
		caliper.complete = true;

		const smoothed = smooth(profile, opts.filterHalfWidth);
		const peak = peakGradient(smoothed);
		caliper.peakContrast = Math.abs(peak);
		peaks.push(caliper.peakContrast);
		if (caliper.peakContrast > peakContrast) {
			peakContrast = caliper.peakContrast;
			peakStrength = peak;
		}

		const edge = selectEdge(findEdges(smoothed, opts), opts);
		if (!edge) continue;
		found++;
		...
		const point = {
			x: startX + frame.scan.x * u,
			y: startY + frame.scan.y * u,
		...
		points.push(point);
		caliper.edge = { x: point.x, y: point.y };
		caliper.used = true;
	}

	peaks.sort((a, b) => a - b);
	const half = peaks.length >> 1;
	const diagnostics = {
		peakContrast,
		peakPolarity:
			peakContrast === 0 ? null : peakStrength > 0 ? "darkToLight" : "lightToDark",
		medianPeakContrast:
			peaks.length === 0
				? 0
				: peaks.length % 2
					? peaks[half]
					: (peaks[half - 1] + peaks[half]) / 2,
		calipersWithEdge: found,
		calipersInImage: peaks.length,
	};
	...
	for (const c of caliperLines) if (c.edge) c.used = usedSet.has(c.band);
	...
	const midV = { x: frame.origin.x + frame.line.x * halfLength, y: frame.origin.y + frame.line.y * halfLength };
	...
	return {
		...
		line: {
			...
			dx: line.dx,
			dy: line.dy,
			p0: { x: mid.x - line.dx * halfLength, y: mid.y - line.dy * halfLength },
			p1: { x: mid.x + line.dx * halfLength, y: mid.y + line.dy * halfLength },
		},
		angleDeg,
		score,
