...
async function resize(image, wMode, wVal, hMode, hVal, fmt, quality) {
	...
	const cv = await ready();
	const raw = describeRaw(image);
	let width = resolveDimension(wMode, wVal, raw.width);
	let height = resolveDimension(hMode, hVal, raw.height);
	// 0 / NaN on one axis keeps the source aspect ratio, as the native
	// engine's resize.cpp treats it; label-crop's preview relies on that.
	if (width === null && height === null) {
		throw new Error("cvjs: resize needs at least one positive size");
	}
	if (width === null) {
		width = Math.max(1, Math.round((height * raw.width) / raw.height));
	}
	if (height === null) {
		height = Math.max(1, Math.round((width * raw.height) / raw.width));
	}
	const s = scope();
	try {
		const src = toMat(cv, s, image);
