...
function resolveDimension(mode, value, source) {
	...
	if (mode === "pct" || mode === "percent" || mode === "scale") {
		return Math.max(1, Math.round(source * (n > 5 ? n / 100 : n)));
	}
...
async function filter(image, type, kernel, intensity, fmt, quality) {
	...
	try {
		...
		if (kind === "otsu") {
			const k =
				Number.isFinite(kernel) && kernel >= 3 ? Math.round(kernel) | 1 : 3;
...
module.exports = {
	...
	_internals: { scope, toMat, fromMat, describeRaw, timing, matTypeFor },
	_reset,
};
