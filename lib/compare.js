...
function renderGrayPng(gray, width, height, cfg) {
...
function renderMaskPng(mask, width, height, cfg) {
...
async function prepareGolden(buffer, cfg) {
	...
	const stages = cfg.debugStages
		? await allProps({
				goldenGray: renderGrayPng(pixels, width, height, cfg),
				goldenFg: renderMaskPng(fg, width, height, cfg),
				goldenFgDilatedBackground: renderMaskPng(
					fgDilatedBackground,
					width,
					height,
					cfg,
	...
	if (cfg.debugStages) {
		...
		stages = {
			...
			...(await allProps({
				...
				targetGray: renderGrayPng(
					targetGray,
					targetWorking.width,
					targetWorking.height,
					cfg,
				),
				targetFg: renderMaskPng(
					targetFg,
					targetWorking.width,
					targetWorking.height,
					cfg,
				...
				targetGrayAligned: renderGrayPng(alignedTargetGray, width, height, cfg),
				targetFgAligned: renderMaskPng(alignedTargetFg, width, height, cfg),
				targetFgDilatedPrint: renderMaskPng(
					targetFgDilatedPrint,
					width,
					height,
					cfg,
				),
				printDefect: renderMaskPng(printDefect, width, height, cfg),
				backgroundDefect: renderMaskPng(backgroundDefect, width, height, cfg),
			})),
		};
	}
