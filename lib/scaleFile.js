...
const { validateHomography } = require("./homography.js");
 ...
 *
 * `homography` (the plane homography checkerboard-calibrate measured, in
 * the calibration photo's native pixels) is likewise validated only when
 * present. Only perspective-rectify needs it, and it refuses a file
 * without one; golden-compare and the rest ignore it.
 */
async function readScaleFile(scaleFilePath) {
	...
	if (parsed.homography != null) {
		const reason = validateHomography(parsed.homography);
		if (reason) {
			return {
				error: `calibration ${reason} - re-run checkerboard-calibrate`,
			};
		}
		if (w == null || h == null) {
			return {
				error:
					"calibration has a homography but no nativeWidth/nativeHeight " +
					"to express it against - re-run checkerboard-calibrate",
			};
		}
	}
	return parsed;
...
module.exports = { readScaleFile, writeScaleFile, pathExists, isFinitePositive };
