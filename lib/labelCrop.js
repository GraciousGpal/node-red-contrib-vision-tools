...
function analyzeMask(mask, width, height, cfg) {
	...
	// Before this a miss carried zeros for every field the gate had not
	// reached, and a NaN dominance.
	const measured = {};
...
function refineRectBoundary(grayData, edgeData, width, height, rect, polarity) {
	...
	const unchanged = () => ({
		...
		h,
		theta,
		sides: ["left", "right", "top", "bottom"].map((side) => ({
			side,
			from: { left: u0, right: u1, top: v0, bottom: v1 }[side],
			to: { left: u0, right: u1, top: v0, bottom: v1 }[side],
			snapped: false,
		})),
	});
	...
	if (newW < 8 || newH < 8) return unchanged();
	...
	return {
		...
		theta,
		sides: [
			{ side: "left", from: u0, to: left, snapped: left !== u0 },
			{ side: "right", from: u1, to: right, snapped: right !== u1 },
			{ side: "top", from: v0, to: top, snapped: top !== v0 },
			{ side: "bottom", from: v1, to: bottom, snapped: bottom !== v1 },
		],
	};
}
...
function buildMissMetadata(analysis, timings, smallW, smallH, sx, sy) {
	return {
		...
		areaFraction: analysis ? measuredOrNull(analysis.areaFraction, round4) : null,
		rectangularity: analysis ? measuredOrNull(analysis.rectangularity, round3) : null,
		dominance: analysis ? measuredOrNull(analysis.dominance, round3) : null,
		borderContact: analysis ? measuredOrNull(analysis.borderContact, round3) : null,
		...(analysis && analysis.aspect != null ? { aspect: round3(analysis.aspect) } : {}),
