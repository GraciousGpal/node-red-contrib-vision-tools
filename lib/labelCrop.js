...
const DEFAULTS = {
	...
	// "upstream" detects nothing and crops the rectangle a line-finder
	// ahead of this node already measured (`rect`)
	boundaryMode: "blob",
	// calipers mode only: { left, right, top, bottom }, each an object of
	// { x, y, width, height, angleDeg? } plus any lib/lineFinder.js option
	edgeRegions: null,
	// upstream mode only: the rectangle to crop, as line-finder reports it
	// in msg.lineFinder.rect - { ok, reason, center: { x, y }, width,
	// height, angleDeg, corners?, score?, residualPx? } in full-frame
	// pixels. Left as given: a malformed one is reported as a miss.
	rect: null,
...
function exteriorRect(px, width, height) {
	...
	// [minU, maxU, minV, maxV] of the hull in axes rotated by (ca, sa)
	const extents = (ca, sa) => {
		let minU = Infinity;
		let maxU = -Infinity;
		let minV = Infinity;
	...
	for (let i = 0; i < hull.length; i++) {
		...
		const [minU, maxU, minV, maxV] = extents(Math.cos(theta), Math.sin(theta));
	...
	const [minU, maxU, minV, maxV] = extents(ca, sa);
...
function analyzeMask(mask, width, height, cfg) {
	...
	// Every gate reports what it measured, so a miss says which setting to
	// move and by how much: "border-contact" with borderContact 0.75
	// against a 0.5 limit is actionable, "border-contact" alone is not.
	const measured = {};
	const miss = (reason) => ({ detected: false, reason, confidence: 0, ...measured });

	if (comps.length === 0) return miss("no-component");
	const largest = comps.reduce((a, b) => (b.area > a.area ? b : a));
	measured.areaFraction = largest.area / frameArea;
	const largeEnough = comps.filter((c) => c.area >= minArea);
	if (largeEnough.length === 0) return miss("too-small");
	const candidates = largeEnough.filter((c) => c.area <= maxArea);
	if (candidates.length === 0) return miss("too-large");
	candidates.sort((a, b) => b.area - a.area);
	const best = candidates[0];
	const second = candidates[1];
	measured.areaFraction = best.area / frameArea;
	const dominance = second ? best.area / second.area : Infinity;
	measured.dominance = dominance;
	if (dominance < opts.minDominance) return miss("ambiguous");
	...
	measured.borderContact = borderContact;
	if (borderContact > opts.maxBorderContact) return miss("border-contact");

	const px = collectPixels(mask, width, height, best.seed);
	const rect = exteriorRect(px, width, height);
	if (!rect || rect.w <= 0 || rect.h <= 0) return miss("degenerate-boundary");
	const areaFraction = measured.areaFraction;
	const rectangularity = rect.area / (rect.w * rect.h);
	measured.rectangularity = rectangularity;
	if (rectangularity < opts.minRectangularity) return miss("low-rectangularity");

	let aspectMismatch = 0;
	if (opts.aspectRatio != null) {
		const aspect = rect.w / rect.h;
		measured.aspect = aspect;
		aspectMismatch = Math.abs(Math.log(aspect / opts.aspectRatio));
		if (aspectMismatch > opts.aspectTolerance) return miss("aspect-mismatch");
	}
	...
	if (confidence < opts.minConfidence) {
		return { ...miss("low-confidence"), confidence };
	}
 ...
 * found scanning inward from each side. Grey tone is accumulated into 1-D
 * profiles along the rect's axes (u = along theta, v = perpendicular),
 * each bin the mean tone of the pixels within the rect's perpendicular
 * extent - inverted for a dark label, so "label" is always the bright
 * side below.
 *
 * The boundary is a **step**: a place where the mean tone rises by at
 * least `minStep` across `k` bins going inward, and where everything
 * between it and the region side is darker than the interior just past
 * it. The innermost such step wins. Three things fall out of that:
 *
 *  - a bright halo on the table (the 13112025_003 fixture: label 255,
 *    halo 238, table 33) has *two* steps, table->halo and halo->label;
 *    the halo->label one is innermost and its outside strip - the halo -
 *    is darker than the label, so that is the boundary;
 *  - a printed band inside the label (a barcode, a rule) also makes a
 *    step on its inner side, but the strip outside it contains the
 *    label's own white margin, as bright as the interior, so it is
 *    refused. This is what the production run needed: the previous
 *    criterion ("label tone" = the frame's brightest 2%, 246+) called the
 *    whole vignetted left third of the label - 214 rising to 250 -
 *    "not label" and walked 100 columns in, past the barcode, on three
 *    frames in four. A step rule does not care what absolute level the
 *    label sits at, only that its edge is an edge and a lighting ramp is
 *    not;
 *  - a side clipped at the frame has no outside to step from, and the
 *    strip from the frame edge inward is label tone, so it stays put.
 *
 * Sobel magnitude is the fallback for a seam on an equal-tone surface
 * (a label bottom on a bright table), under the same strip rule so a
 * printed band is never taken for the edge.
 *
 * A side with no evidence keeps its region position.
 *
 * @param {Uint8Array} grayData small gray copy
...
function refineRectBoundary(grayData, edgeData, width, height, rect, polarity) {
	...
	const edgeFloor = 24;
	// a boundary is a rise of at least this many grey levels across `k`
	// bins; a vignetting ramp on this project's frames is ~0.6 per bin
	const minStep = 12;
	const k = 3;
	// bins with fewer samples than this are unknown, not zero
	const minCount = 8;
	const dark = polarity === "dark";

	// Per-axis accumulators. Tone is taken over the central 60% of the
	// rect's perpendicular extent: the label always covers the rect's
	// centre, while the outer rows/columns hold whatever the other two
	// sides have yet to trim (halo, table), and a synthetic label with 11
	// background rows in its column means lost a 15-level halo step to
	// dilution. Edges keep the full extent - a boundary line runs the
	// whole side, and the floor is scaled by that length.
	const bandU0 = vc - 0.3 * h;
	const bandU1 = vc + 0.3 * h;
	const bandV0 = uc - 0.3 * w;
	const bandV1 = uc + 0.3 * w;
	const countU = new Int32Array(accSize);
	const countV = new Int32Array(accSize);
	const sumU = new Float64Array(accSize);
	const sumV = new Float64Array(accSize);
	...
	const meanU = new Float64Array(accSize).fill(NaN);
	const meanV = new Float64Array(accSize).fill(NaN);
	for (let i = 0; i < accSize; i++) {
		if (countU[i] >= minCount) meanU[i] = sumU[i] / countU[i];
		if (countV[i] >= minCount) meanV[i] = sumV[i] / countV[i];
	}
	...
	// mean of k known bins from p stepping by dir; NaN if any is unknown
	const run = (mean, p, dir) => {
		let s = 0;
		for (let j = 0; j < k; j++) {
			const v = mean[p + j * dir + off];
			if (Number.isNaN(v)) return NaN;
			s += v;
		}
		return s / k;
	};
	// brightest known bin strictly between the region side and p
	const stripMax = (mean, start, p, dir) => {
		let max = -Infinity;
		for (let q = start; q * dir < p * dir; q += dir) {
			const v = mean[q + off];
			if (!Number.isNaN(v) && v > max) max = v;
		}
		return max;
	};
	// The innermost step whose outside strip is uniformly darker than the
	// interior just past it. Scanned outer to inner; the last hit wins.
	const scanStep = (start, dir, mean) => {
		let found = null;
		for (let i = 0; i <= win; i++) {
			const p = start + dir * i;
			if (p + (k - 1) * dir + off < 0 || p + (k - 1) * dir + off >= accSize) break;
			const inside = run(mean, p, dir);
			const outside = run(mean, p - k * dir, dir);
			if (Number.isNaN(inside) || Number.isNaN(outside)) continue;
			if (inside - outside < minStep) continue;
			if (stripMax(mean, start, p, dir) > inside - minStep) continue;
			found = p;
		}
		return found;
	};
	// Sobel seam: the outermost run of three strong bins, accepted only if
	// the strip outside it is darker than the interior past it - a printed
	// band inside the label reads exactly like a seam on the accumulator.
	const scanEdge = (start, dir, edge, floor, mean) => {
		for (let i = 0; i <= win; i++) {
			const p = start + dir * i;
			...
			if (a >= floor && b >= floor && c >= floor) {
				const inside = run(mean, p + 3 * dir, dir);
				if (Number.isNaN(inside)) return null;
				const outsideMax = stripMax(mean, start, p, dir);
				if (outsideMax === -Infinity || outsideMax <= inside - minStep / 2) {
					return p;
				}
				return null;
			}
	...
	// boundary rounding, and a scan that merely confirms the region
	// position is no snap at all. Tone step first, Sobel seam second.
	const improve = (region, step, edge) => {
		if (step !== null && Math.abs(step - region) >= 3) return step;
		if (edge !== null && Math.abs(edge - region) >= 3) return edge;
		return region;
	};
	const left = improve(
		u0,
		scanStep(u0, 1, meanU),
		scanEdge(u0, 1, edgeU, floorU, meanU),
	);
	const right = improve(
		u1,
		scanStep(u1, -1, meanU),
		scanEdge(u1, -1, edgeU, floorU, meanU),
	);
	const top = improve(
		v0,
		scanStep(v0, 1, meanV),
		scanEdge(v0, 1, edgeV, floorV, meanV),
	);
	const bottom = improve(
		v1,
		scanStep(v1, -1, meanV),
		scanEdge(v1, -1, edgeV, floorV, meanV),
	);

	const sides = (l, r, t, b) => [
		{ side: "left", from: u0, to: l, snapped: l !== u0 },
		{ side: "right", from: u1, to: r, snapped: r !== u1 },
		{ side: "top", from: v0, to: t, snapped: t !== v0 },
		{ side: "bottom", from: v1, to: b, snapped: b !== v1 },
	];
	const newW = right - left;
	const newH = bottom - top;
	// Degenerate snap (a huge window crossed the opposite side): revert.
	if (newW < 8 || newH < 8) {
		return { cx, cy, w, h, theta, sides: sides(u0, u1, v0, v1) };
	}
...
// A field the miss never measured is null, not 0: a 0 reads as "measured
// and found nothing", which for borderContact or dominance is the opposite
// of what happened.
const measuredOrNull = (v, r) =>
	typeof v === "number" && Number.isFinite(v) ? r(v) : null;
...
function buildMissMetadata(analysis, timings, smallW, smallH, sx, sy) {
	return {
		...
		areaFraction: measuredOrNull(analysis.areaFraction, round4),
		rectangularity: measuredOrNull(analysis.rectangularity, round3),
		dominance: measuredOrNull(analysis.dominance, round3),
		borderContact: measuredOrNull(analysis.borderContact, round3),
		...(analysis.aspect != null ? { aspect: round3(analysis.aspect) } : {}),
		...
		...(analysis.edges ? { edges: analysis.edges } : {}),
		timings,
	};
}
...
async function labelCrop(input, cfg = {}, engine) {
	...
	// ---- calipers and upstream boundary modes -------------------------
	...
	// artwork's - or no search at all, when a line-finder ahead of this
	// node has already measured the rectangle and it arrives on the
	// message. Both produce the same `best` shape the blob path does, so
	// the rotate/crop tail below is shared unchanged.
	if (opts.boundaryMode === "calipers" || opts.boundaryMode === "upstream") {
		const found =
			opts.boundaryMode === "calipers"
				? await boundaryByCalipers(eng, full, opts, timings)
				: boundaryFromRect(full, opts.rect);
...
/**
 * Boundary from a rectangle measured upstream.
 *
 * A line-finder with regions named left, right, top and bottom reports
 * the rectangle they bound as msg.lineFinder.rect, built by the same
 * rectFromLines the calipers path calls. Nothing is detected here: the
 * rect is checked for shape and put into the same `best` the calipers
 * path builds, so the crop, the metadata and the gates - there are none
 * between a found rectangle and the crop, in either mode - come out as
 * they would had calipers measured it on this frame.
 *
 * A rect that is absent, carries the line-finder's own miss, or is
 * malformed is a miss with a reason that says which - the frame passes
 * through - never an error and never a crop against a guessed rectangle.
 * The centre is accepted as `center: { x, y }` (the node's shape) or as
 * `cx`/`cy` (rectFromLines' own); corners are optional and computed in
 * the same tl, tr, br, bl order when missing.
 */
function boundaryFromRect(full, rect) {
	const smallW = full.width;
	const smallH = full.height;
	const fail = (reason) => ({
		best: { detected: false, reason, confidence: 0, polarity: null },
		smallW,
		smallH,
	});
	const finite = (v) => typeof v === "number" && Number.isFinite(v);

	if (rect == null) return fail("no-upstream-rect");
	if (typeof rect !== "object" || Array.isArray(rect)) {
		return fail("bad-upstream-rect");
	}
	if (rect.ok === false) {
		const why =
			typeof rect.reason === "string" && rect.reason ? rect.reason : "not-ok";
		return fail(`upstream-rect:${why}`);
	}
	if (rect.ok !== true) return fail("bad-upstream-rect");

	const centre =
		rect.center && typeof rect.center === "object"
			? rect.center
			: { x: rect.cx, y: rect.cy };
	const cx = centre.x;
	const cy = centre.y;
	const { width, height, angleDeg } = rect;
	if (![cx, cy, width, height, angleDeg].every(finite)) {
		return fail("bad-upstream-rect");
	}
	if (width < 2 || height < 2) return fail("bad-upstream-rect");
	if (cx < 0 || cx > smallW || cy < 0 || cy > smallH) {
		return fail("bad-upstream-rect");
	}

	let corners = null;
	if (
		Array.isArray(rect.corners) &&
		rect.corners.length === 4 &&
		rect.corners.every((c) => c && finite(c.x) && finite(c.y))
	) {
		corners = rect.corners.map((c) => ({ x: c.x, y: c.y }));
	} else {
		// x = cx + u*ca - v*sa, y = cy + u*sa + v*ca, with u along the
		// width and v along the height: tl, tr, br, bl
	...
	return {
		...
		best: {
			detected: true,
			reason: "upstream-rect",
			// nothing was thresholded, so there is no polarity to report
			polarity: null,
			// the line-finder's score when it sent one; a rect built by hand
			// carries no evidence against it
			confidence: finite(rect.score) ? rect.score : 1,
			center: { x: cx, y: cy },
			width,
			height,
			angleDeg,
			corners,
			areaFraction: (width * height) / (smallW * smallH),
			rectangularity: null,
			dominance: null,
			borderContact: null,
			snappedSides: [],
			residualPx: finite(rect.residualPx) ? rect.residualPx : null,
		},
	};
}
 ...
 *
 * Split out because every boundary mode ends here: `best` is in
...
async function cropToRect({
	...
	const rotate = Math.abs(angleDeg) >= opts.minRotateAngleDeg;
	let rotated = full;
	if (rotate) {
		const d0 = performance.now();
	...
	const metadata = {
		detected: true,
		// "ok" from a detection; "upstream-rect" when the rectangle came in
		// on the message and nothing here measured it
		reason: best.reason,
